...
# by flags on the command line. An unrecognized bare argument is treated as
# a positional SERVICE name rather than rejected as an unknown flag.
# Mirrors scripts/powershell/logs.ps1.
...
service_suffix=""
if [[ -n "${SERVICE}" ]]; then
    service_suffix=" (service: ${SERVICE})"
fi
log_info "Showing logs for '${PROJECT_NAME}'${service_suffix}"

compose_args=(compose -f "${COMPOSE_FILE}" -p "${PROJECT_NAME}" logs --tail "${TAIL_LINES}")
